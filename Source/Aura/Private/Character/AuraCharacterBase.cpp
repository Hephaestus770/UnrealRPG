...
void AAuraCharacterBase::StunTagChanged(const FGameplayTag CallbackTag, int32 NewCount)
{
	...
	if (bIsStunned)
	{
		...
		const FGameplayTagContainer AbilitiesToCancelOnStun(FAuraGameplayTags::Get().Abilities_CancelOnStun);
		GetAbilitySystemComponent()->CancelAbilities(&AbilitiesToCancelOnStun);
...
FVector AAuraCharacterBase::GetCombatSocketLocation_Implementation(const FGameplayTag& MontageTag) // With TMap this function can be less hardcoded!!!
{
	const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
	...
	if (MontageTag.MatchesTagExact(GameplayTags.CombatSocket_WeaponSecondary) && IsValid(Weapon))
	{
...
FTaggedMontage AAuraCharacterBase::GetTaggedMontageByTag_Implementation(const FGameplayTag& MontageTag)
{
	...
	return FTaggedMontage(); // return empty FTaggedMontage
}
...
void AAuraCharacterBase::Dissolve()
{
	if (IsValid(DissolveMaterialInstance))
	{
		...
		GetMesh()->SetMaterial(0, DynamicMatInst); // Mesh can multiple materials, so this needs to be done for each of mesh material
