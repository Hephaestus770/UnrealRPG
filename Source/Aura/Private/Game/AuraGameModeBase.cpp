...
void AAuraGameModeBase::LoadWorldState(UWorld* World) const
{
	...
	{	
		...
		for (FActorIterator It(World); It; ++It)
		{
			AActor* Actor = *It;  // NOTE: Actor is real actor not pointer don't forget!!!!
			...
			for (FSavedActor SavedActor : SaveGame->GetSavedMapWithName(WorldName).SavedActors)
			{
				if (SavedActor.ActorName == Actor->GetName())
				{
					...
					ISaveInterface::Execute_LoadActor(Actor); // this will for handling glowing for checkpoint actors, can be used for other things too!!!
				}
			}
