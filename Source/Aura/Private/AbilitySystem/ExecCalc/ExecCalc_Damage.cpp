	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	float Damage = 0.f;
	for (FGameplayTag DamageTypes : FAuraGameplayTags::Get().DamageTypes)
	{
		const float DamageType = Spec.GetSetByCallerMagnitude(DamageTypes);
	...
	const float CriticalHitResistentCoefficient = CriticalHitResistanceCurve->Eval(TargetCombatInterface->GetPlayerLevel());
